...
function blankDoc() {
  return {
    id: newId(),
    type: "invoice", // "invoice" or "proforma"
    docNo: getNextNumber("invoice"),
...
export default function InvoiceEditor() {
  ...
  const buildPdf = () => generateInvoicePdf(doc_, company);
  const filename = () =>
    `TaxInvoice_${doc_.docNo}_${(doc_.client.name || "client").replace(
      /\s+/g,
      "_",
    )}.pdf`;
  ...
  return (
    <div className="min-h-full bg-steel-50 pb-28">
      <TopBar title="Tax Invoice" subtitle={`Invoice No. ${doc_.docNo}`} />
      ...
      <div className="space-y-4 p-4">
        <div className="card grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="field-label">
              {doc_.type === "proforma" ? "Proforma No" : "Invoice No"}
            </label>
        ...
        <div className="card space-y-3">
          <p className="text-sm font-bold text-steel-800">Client (Bill To)</p>
          <div>
        ...
        <div className="card space-y-1.5">
          <p className="mb-1 text-sm font-bold text-steel-800">
            GST Summary (auto)
          </p>
          <Row label="Sub Total" value={summary.subTotal} />
          <Row label={`C-GST ${company.cgstPercent}%`} value={summary.cgst} />
          <Row label={`S-GST ${company.sgstPercent}%`} value={summary.sgst} />
          <Row label="Round Off" value={summary.roundOff} />
          <div className="my-1 border-t border-steel-100" />
          <Row label="Grand Total" value={summary.grandTotal} bold />
          <p className="pt-1 text-[11px] text-steel-400">
            Edit CGST/SGST % in Company Settings.
          </p>
        </div>

        <div className="card space-y-2">
          <p className="text-sm font-bold text-steel-800">Note</p>
...
function Row({ label, value, bold }) {
  return (
    <div className="flex items-center justify-between">
      <span
        className={`text-xs ${
          bold ? "font-bold text-steel-800" : "text-steel-500"
        }`}
      >
        {label}
      </span>
      <span
        className={`font-mono text-sm ${
          bold ? "font-bold text-steel-900" : "text-steel-700"
        }`}
      >
        ₹{" "}
